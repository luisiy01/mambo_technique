...
export function AttendanceContent() {
  ...
  const handleSave = async () => {
    ...
    if (res.success) {
      toast.success("Pase de lista guardado correctamente", {
        description: `Se registró la asistencia para ${students.length} alumnos.`,
      });
    } else {
      toast.error("Error al guardar asistencia", {
        description:
          res.error || "Ocurrió un problema al conectar con Supabase.",
      });
    }
  };
      ...
      {isLoadingStudents ? (
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
            <div className="text-xs text-slate-600 font-semibold flex items-center gap-2">
              ...
              Sede: {scheduleDetails?.locationName} | Profesor:{" "}
        ...
        <div className="text-center py-16 bg-white rounded-xl border border-slate-200">
          ...
          <p className="text-xs text-slate-400 mt-1">
            Asigna alumnos a este taller desde el módulo de Alumnos para tomar
            lista.
          </p>
        </div>
      )}
